...
const StyledSelect = styled(MantineSelect)`
 ...
 & .mantine-Select-input {
    ${({ error }) => error ? tw`border-error text-error` : tw`border-accent-200 focus:border-accent-500`}
 }

 & .mantine-Select-dropdown {
    ${tw`shadow-lg dark:shadow-none border border-accent-200`}
 }
 ...
 ${({ itemComponent }) => !itemComponent ? css`
    & .mantine-Select-item {
...
const Select = ({ loading, error, ...props}: SelectProps) => (
    <StyledSelect error={error ? <ErrorMessage>{error}</ErrorMessage> : undefined} nothingFound={loading && 'Loading...'} rightSection={loading && <LoadingDots size={4} />} {...props} />
)
