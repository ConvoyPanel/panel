...
const Checkbox = ({ onChange, checked, indeterminate, children, disabled, className }: Props) => {
    const id = useMemo(() => Math.random().toString(36).substring(7), [])

    return (
        <label className={`inline-flex items-center space-x-3 ${disabled && 'cursor-not-allowed'} ${className}`} htmlFor={id}>
            ...
            {children && <span className={`text-sm ${disabled ? 'text-accent-300' : 'text-foreground'}`}>{children}</span>}
        </label>
    )
}
