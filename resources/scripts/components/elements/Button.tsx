...
const Button = styled.button<Props>`
  ...
  ${({ size }) => (size === 'sm' ? tw`px-3 py-1.5` : tw`px-5 h-9`)}
    ...
    ${({ variant = 'outline' }) => variant === 'outline' ? tw`border-accent-200 hover:border-foreground active:border-foreground text-accent-500 hover:text-foreground active:text-foreground active:bg-accent-200 bg-background` : ''}
`
