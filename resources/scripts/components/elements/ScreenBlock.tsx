...
const ScreenBlock = ({ title, icon: Icon, message, center, onBack, onRetry, full, children }: ScreenBlockProps) => {
    return (
        <PageContentBlock className={`${full && 'grid place-items-center min-h-screen -mt-10'}`} title={title}>
            <div
            ...
            >
                ...
                {(onBack || onRetry) && (
                    <div className='flex justify-center mt-3'>
                        <Button onClick={() => (onRetry ? onRetry() : onBack ? onBack() : null)}>
...
export const NotFound = ({
    ...
    <ScreenBlock
        title={title || '404'}
        message={message || "The link is either broken or doesn't exist on the server."}
        icon={LinkIcon}
        full={full}
        onBack={onBack}
    />
