...
const CreatePoolModal = ({ pool, onClose, mutate }: Props) => {
    ...
    const { clearFlashes, clearAndAddHttpError } = useFlashKey(`admin.addressPools.${pool?.id}.delete`)
    const { trigger, isMutating } = useSWRMutation(['admin.address-pools.delete', pool?.id], async () => {
        clearFlashes()
        try {
    ...
    return (
        <Modal open={Boolean(pool)} onClose={onClose}>
            <Modal.Header>
                <Modal.Title>{t('delete_modal.title', { name: pool?.name })}</Modal.Title>
            </Modal.Header>
            ...
            <form onSubmit={submit}>
                ...
                <Modal.Actions>
                    ...
                    <Modal.Action type='submit' disabled={pool ? pool.nodesCount > 0 : false} loading={isMutating}>
                        {tStrings('delete')}
                    </Modal.Action>
                </Modal.Actions>
            </form>
