 ...
 *     await capture(ctx, { url: '/admin', width: 768, path: '/tmp/overview.png' })
 *     await browser.close()
 */
import { chromium } from 'playwright'
import { execFileSync } from 'node:child_process'

export const BASE = process.env.SBX_APP_URL ?? 'https://convoy4x.ddev.site'
 ...
 * convoy4x.ddev.site drives your real host app (leaked sessions, mutated data).
...
export async function login(context, { email, password } = {}) {
    ...
    // `waitUntil: 'commit'` matters here. The default is 'load', and this app
    // leaves /auth/login by a client-side react-router redirect that fires no
    // second load event -- so the default hangs for the full timeout on a login
    // that actually succeeded. 'commit' resolves as soon as the URL changes,
    // which is the only thing this predicate is asking about.
...
export async function capture(context, { url, path, width = 1440, height = 1000, settle = 1000 }) {
    ...
    try {
        ...
        // Awaited, not returned directly: `return <promise>` inside a try lets
        // the finally close the page while evaluate is still in flight, and the
        // call dies with "Target page, context or browser has been closed".
